...
handler.before = async (m) => {
  ...
  if (bot.modoia && !m.isGroup && !Prefijo && !m.fromMe && m.text !== '') {
     if (/^.*false|disnable|(turn)?off|0/i.test(m.text)) return;
        ...
        const sytm = await fetch(`https://raw.githubusercontent.com/KillzN/Barbie-Bot1/master/src/JSON/chatgpt_indicciones.txt`).then(v => v.text());
        const sistema1 = sytm.replace('@name', namedem)
        const sistema2 = 'Tu seras 𝐁𝐀𝐑𝐁𝐈𝐂𝐈𝐓𝐀 𝐁𝐎𝐓 - MD, un bot de WhatsApp creado por KillzN';
        try {
